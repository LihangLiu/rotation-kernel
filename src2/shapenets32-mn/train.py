...
def train(net, optimizer, no_epoch, loader):
    net.train()
    loss_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alter_step_size', default = 1, type = int)
    ...
    if 'rot' in args.kernel_mode:
        lr_schedulers = [
            ...
            StepLR(optimizer.optim1, step_size=args.lr_step_size, gamma=0.3, last_epoch=-1),
        ]
    ...
    for epoch in range(start_iter, args.max_iter): 
        ...
        ### train
        if 'rot' in args.kernel_mode:
            optimizer.set_optim(int(epoch / args.alter_step_size) % 2)
        ...
        ### output losses
        msg = "{0} {1:.6f} {2:.6f} {3:.6f}".format(
            epoch, train_loss, accuracy['valid'], accuracy['test']
        )
